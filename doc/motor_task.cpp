...
void MotorTask::run(){
    ...
    while (1)
    {
       ...
       bool out_of_bounds = config.num_positions > 0 && ((angle_to_detent_center > 0 && config.position == 0) || (angle_to_detent_center < 0 && config.position == config.num_positions - 1));
        motor.PID_velocity.limit = 2; //out_of_bounds ? 10 : 3;
        //motor.PID_velocity.P = out_of_bounds ? abs(tcp_force)  : 0.7 * config.detent_strength_unit;
        motor.PID_velocity.P = out_of_bounds ? 0.95  : 0.7 * config.detent_strength_unit;
        ...
        if (fabsf(motor.shaft_velocity) > 60) {
            ...
            //float torque =  motor.PID_velocity(-0.1*tcp_force);
            float torque =  -0.095*tcp_force;
            #if SK_INVERT_ROTATION
            torque = -torque;
            #endif
            motor.move(torque);  //test physical meaning 
