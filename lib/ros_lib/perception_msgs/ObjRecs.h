#ifndef _ROS_perception_msgs_ObjRecs_h
#define _ROS_perception_msgs_ObjRecs_h
...
#include "perception_msgs/ObjRec.h"

namespace perception_msgs
{

  class ObjRecs : public ros::Msg
  {
    public:
      typedef std_msgs::Header _header_type;
      _header_type header;
      uint32_t objects_length;
      typedef perception_msgs::ObjRec _objects_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(objects_lengthT > objects_length)
        this->objects = (perception_msgs::ObjRec*)realloc(this->objects, objects_lengthT * sizeof(perception_msgs::ObjRec));
      objects_length = objects_lengthT;
      for( uint32_t i = 0; i < objects_length; i++){
      offset += this->st_objects.deserialize(inbuffer + offset);
        memcpy( &(this->objects[i]), &(this->st_objects), sizeof(perception_msgs::ObjRec));
      }
     return offset;
    }

    virtual const char * getType() override { return "perception_msgs/ObjRecs"; };
    virtual const char * getMD5() override { return "108d0591125b46da962c96dea57ca8ac"; };
