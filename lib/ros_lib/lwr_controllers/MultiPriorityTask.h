  ...
  class MultiPriorityTask : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const override
    {
      ...
      *(outbuffer + offset + 3) = (this->tasks_length >> (8 * 3)) & 0xFF;
      ...
      offset += serializeAvrFloat64(outbuffer + offset, this->tasks[i]);
      }
      return offset;
    }
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(tasks_lengthT > tasks_length)
        this->tasks = (float*)realloc(this->tasks, tasks_lengthT * sizeof(float));
      tasks_length = tasks_lengthT;
      for( uint32_t i = 0; i < tasks_length; i++){
      offset += deserializeAvrFloat64(inbuffer + offset, &(this->st_tasks));
        memcpy( &(this->tasks[i]), &(this->st_tasks), sizeof(float));
      }
     return offset;
    }

    virtual const char * getType() override { return "lwr_controllers/MultiPriorityTask"; };
    virtual const char * getMD5() override { return "42e011fbf13e33d6ed6e65ac7a0bdf63"; };
