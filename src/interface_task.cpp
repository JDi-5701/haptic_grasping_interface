...
// WITH_OPCUA: if true, the Smart Knob run with the OPC UA bridge. Default is true.
#define MODE_OPCUA 1
#define MODE_WIFI 2

#define CURRENT_MODE MODE_WIFI
...
extern knob_robot_control::KnobState knob_state_msg;
extern ros::Publisher knob_state_publisher;

// dummy tasks
// static WifiTask dummy_wifi_task(0);
// static UARTTask dummy_uart_task(0);
...
// InterfaceTask::InterfaceTask(const uint8_t task_core, MotorTask& motor_task, UARTTask& uart_task): 
//         Task("Interface", 3000, 1, task_core),
//         stream_(),
//         motor_task_(motor_task),
//         uart_task_(uart_task),
//         wifi_task_(dummy_wifi_task),
...
//     wifi_queue_if = xQueueCreate(1, sizeof(PB_SmartKnobConfig));
//     assert(wifi_queue_if != NULL);

//     uart_queue_if = xQueueCreate(1, sizeof(PB_SmartKnobConfig));
//     assert(uart_queue_if != NULL);
// }
...
void InterfaceTask::run() {
    stream_.begin();
    // here running the initialization of the sensors
    #if SK_LEDS
    ...
    motor_task_.setConfig(configs[7]);
    motor_task_.addListener(knob_state_queue_);

    // Set up the UART task
    // uart_task_.addListener(uart_queue_if);
    wifi_task_.addListener(wifi_queue_if);
...
# if CURRENT_MODE == MODE_OPCUA
            if (millis() - last_publish > 100) {
                // Knob -> OPC UA
                SerialPort.printf("pos: %d, force: %f \n", state.current_position, state.current_force); // use SerialPort to send message to OPC Bridge
                last_publish = millis();
            }
...
# if CURRENT_MODE == MODE_OPCUA
        PB_SmartKnobConfig uartConfig_if;
        if (xQueueReceive(uart_queue_if, &uartConfig_if, 0) == pdTRUE) {
            // Handle UART config, OPC UA -> Knob

            Serial.printf("Received config from UART");

            PB_SmartKnobConfig config;
            config.num_positions = uartConfig_if.num_positions;
            config.position = uartConfig_if.position;
            config.position_width_radians = uartConfig_if.position_width_radians;
            config.detent_strength_unit = uartConfig_if.detent_strength_unit;
            config.endstop_strength_unit = uartConfig_if.endstop_strength_unit;
            config.snap_point = uartConfig_if.snap_point;
            motor_task_.setConfig(config);

        }

# elif CURRENT_MODE == MODE_WIFI
        PB_SmartKnobConfig wifiConfig_if;
        if (xQueueReceive(wifi_queue_if, &wifiConfig_if, 0) == pdTRUE) {
            // Handle Wifi config, Wifi -> Knob

            PB_SmartKnobConfig config;
            config.num_positions = wifiConfig_if.num_positions;
            config.position = wifiConfig_if.position;
            config.position_width_radians = wifiConfig_if.position_width_radians;
            config.detent_strength_unit = wifiConfig_if.detent_strength_unit;
            config.endstop_strength_unit = wifiConfig_if.endstop_strength_unit;
            config.snap_point = wifiConfig_if.snap_point;
            motor_task_.setConfig(config);
        }
# endif
...
void InterfaceTask::updateHardware() {
    ...
    // here to read the ADC sensor to detect the button press
    #if SK_STRAIN
